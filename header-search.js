              ...
              <a href="restaurace-detail.html?id=${identifier}" class="block p-3 hover:bg-white/10 transition border-b border-white/10 last:border-b-0">
                <div class="flex gap-3 items-center">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-white/60 flex items-center gap-2">
                      <span class="flex-1 truncate">${r.city}</span>
                      <button type="button" data-restaurant-id="${identifier}" class="save-restaurant-btn opacity-80 hover:opacity-100 transition" onclick="event.preventDefault(); event.stopPropagation(); window.GurmaoCollections?.toggle('${identifier}').then(() => { const newSaved = window.GurmaoCollections.isSaved('${identifier}'); this.innerHTML = newSaved ? '<svg class=\\'w-6 h-6 text-red-500\\' fill=\\'currentColor\\' viewBox=\\'0 0 24 24\\'><path d=\\'M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z\\'/></svg>' : '<svg class=\\'w-6 h-6 text-red-500\\' fill=\\'none\\' stroke=\\'currentColor\\' viewBox=\\'0 0 24 24\\'><path stroke-linecap=\\'round\\' stroke-linejoin=\\'round\\' stroke-width=\\'2\\' d=\\'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z\\'/></svg>'; this.title = newSaved ? 'Odebrat z oblíbených' : 'Přidat do oblíbených'; });" title="${isSaved ? 'Odebrat z oblíbených' : 'Přidat do oblíbených'}">
