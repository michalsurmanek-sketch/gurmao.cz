import { getSavedRestaurants, unsaveRestaurant, saveRestaurant } from './supabase-client.js';
...
function makeCard(item) {
  ...
  imageLink.className = 'saved-image-wrap';
  ...
  if (restaurant.vibe) imageLink.appendChild(textElement('span', 'vibe-badge', restaurant.vibe));
...
function render() {
  ...
  if (!list.length) {
    const state = textElement('div', 'collection-state', 'V tomto filtru není žádná uložená restaurace.');
...
async function undoRemove() {
  if (!lastRemoved || undoButton.disabled) return;
  undoButton.disabled = true;
  try {
    await saveRestaurant(lastRemoved.slug);
...
async function load() {
  ...
  try {
    rows = await getSavedRestaurants();
