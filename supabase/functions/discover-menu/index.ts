...
async function getHtml(url:string){
  ...
  try{const r=await fetch(url,{redirect:'follow',signal:controller.signal,headers:{'user-agent':'GURMAO-menu-discovery/1.0 (+https://gurmao.cz)'}});if(!r.ok)return null;const type=r.headers.get('content-type')||'';if(!type.includes('text/html')&&!type.includes('text/plain'))return null;return {html:await r.text(),finalUrl:r.url};}finally{clearTimeout(timer);}
}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(!restaurant_id)throw new Error('Chybí restaurant_id');
    const admin=createClient(supabaseUrl,serviceKey);
    const {data:r,error}=await admin.from('restaurants').select('*').eq('id',restaurant_id).single();
    ...
    const homepage=await getHtml(website);
    if(!homepage)throw new Error('Oficiální web se nepodařilo načíst');
    ...
    for(const candidate of ranked){
      const page=await getHtml(candidate.url);if(!page)continue;
      const body=text(page.html);const combined=candidate.score+scoreCandidate(page.finalUrl,candidate.label,body);
      if(/denn[ií]\s+menu|poledn[ií]\s+menu|menu\s+dne|ob[eě]dov[eé]\s+menu|pol[eé]vka|hlavn[ií]\s+j[ií]dlo/i.test(body)&&(!best||combined>best.score))best={url:page.finalUrl,score:combined};
    }
    if(!best)return new Response(JSON.stringify({message:'Na webu nebyla nalezena stránka s denním menu'}),{status:404,headers:{...corsHeaders,'Content-Type':'application/json'}});

    const {error:updateError}=await admin.from('restaurants').update({menu_url:best.url,menu_source:'website',menu_auto_enabled:true,menu_last_checked:new Date().toISOString()}).eq('id',restaurant_id);
    if(updateError)throw updateError;
    await admin.from('menu_import_queue').upsert({restaurant_id,status:'pending',scheduled_for:new Date().toISOString(),last_error:null},{onConflict:'restaurant_id'});
    return new Response(JSON.stringify({menu_url:best.url,score:best.score}),{headers:{...corsHeaders,'Content-Type':'application/json'}});
